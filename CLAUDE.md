...
- **Typography**: chunky 8-bit pixel type. `Press Start 2P` for display/headings and the logo (short strings only — it's wide and hard to read in long runs). `Pixelify Sans` for body/UI text — pixel-styled but legible at small mobile sizes. Both self-hosted via `@fontsource`, no runtime font requests.
