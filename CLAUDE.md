...
- **Typography**: `DotGothic16` (self-hosted from `public/fonts/DotGothic16-Modified.{woff2,ttf}`) is the one font, used for both display and body — a personal, kerning-tuned fork of Google's DotGothic16, brought over from the japanese-study repo. Dot-matrix pixel style reads closer to a real station departure board than a game font, and it covers Japanese glyphs, which matters once `name_ja` shows up in the UI. Don't add `@fontsource`/Google Fonts packages back in — this font is the single source for the pixel type identity.
